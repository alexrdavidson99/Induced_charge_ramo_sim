...
def load_and_group_sum(DATA_DIR,shift, round_decimals=4):
    ...
    files = list(DATA_DIR.glob(f"induced_charge_tom_P*off*{shift}*.csv"))
...
def extract_z_value(s):
    match = re.search(r"z_(-?\d+)", s)
    if match:
...
Path_to_induced_charge = 'C:/Users/lexda/PycharmProjects/Induced_charge_ramo_sim/induced_current/1000V/'
...
plt.xlabel('Time (ns)')
plt.ylabel('Peak current (mA) test')
...
# plt.scatter(peaks_positions_array, peaks_array*1e3,  label='Data from simulation')
